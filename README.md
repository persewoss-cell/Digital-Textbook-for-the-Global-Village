...
1. **선생님**: 첫 화면에서 "방 만들기"를 눌러 학년/반, 자기 이름, 4자리 숫자 비밀번호를 정해요.
   그러면 방이 하나 생기고, 첫 화면에 학년·반 순서로 정렬된 사각 박스로 표시돼요.
2. **학생**: 첫 화면에서 자기 반 방을 찾아 "학생 참여하기"를 누르고, 번호·이름·선생님이 알려준
   비밀번호를 입력하면 바로 디지털 교과서로 들어가요. (별도 가입/승인 없음)
3. 학생이 교과서에서 한 필기·메모·책갈피·진도는 **그 방이 삭제되기 전까지** 계속 남아있고,
   같은 방에 같은 번호로 다시 들어오면 이어서 볼 수 있어요.
4. **선생님**은 방 박스의 "선생님" 버튼(비밀번호 재입력)으로 들어가 반 학생들의 진도를 보고,
   특정 학생의 학습 화면을 그대로 열람할 수 있어요. 방을 다 쓰면 "방 삭제하기"로 없앨 수 있어요.
5. **관리자**는 첫 화면의 "관리자" 링크에서 **마스터 비밀번호(기본값 7279)**를 입력하면, 개별 방
   비밀번호를 몰라도 모든 방을 열람/관리할 수 있고, 교과서 PDF 업로드도 이 화면에서 해요.

## 디지털 교과서 뷰어 기능

- 책처럼 좌우로 펼쳐지는 페이지 넘김 애니메이션 (react-pageflip 기반)
- **두 쪽 보기 / 한 쪽 크게 보기** 전환, 확대/축소, 쪽 번호 이동
- 왼쪽 **목차** 패널(단원별 이동), **책갈피** 목록
- 오른쪽 **노트(입력란)** 패널 — 쪽마다 자동 저장되는 메모장
- 펜/형광펜 필기 도구(여러 색상, 지우기) — 학생별로 저장되어 다시 펼치면 그대로 보임
- 교재 내 **검색**(실제 PDF 텍스트 기반, 결과 클릭 시 이동), **읽어주기(TTS)**
...
src/
  pages/
    ...
    AdminMasterPage.tsx      # 관리자: 마스터 비밀번호, 전체 방 목록 + 교과서 업로드
    RoomTextbookHome.tsx     # 방 안 교과서 목록
...
- 방을 삭제하면 그 방과 관련된 모든 필기/노트/책갈피/진도 기록이 함께 삭제돼요(되돌릴 수 없음).
